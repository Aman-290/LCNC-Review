...
\begin{abstract}
Low-code and no-code (LCNC) platforms have moved from peripheral rapid-application tools to an increasingly important part of contemporary software engineering, especially as visual modelling, citizen development, and generative AI begin to converge. This paper presents a PRISMA-informed review of LCNC research with emphasis on benefits, challenges, and future directions. The review was constructed from a 170-record corpus assembled from an initial seed set and supplementary academic web searching, then screened to a final set of 50 included studies. Data extraction covered study metadata, method, platform or domain context, quality appraisal, evidence quotes, thematic codes, and synthesis notes. The strongest recurring themes were platform comparison and AI automation, while education, enterprise adoption, usability, integration, model-driven roots, and citizen development formed the next layer of findings. Reported benefits include rapid prototyping, broader participation in software development, business-IT collaboration, education, and AI-assisted workflow generation. However, the reviewed studies also show recurring concerns about governance, shadow IT, security and privacy, scalability, maintainability, platform lock-in, integration, testing, and the unclear boundary between citizen and professional development. To support auditability, the review links its synthesis to public extraction artifacts and a claim-to-evidence map. We conclude with a research agenda that prioritizes governance frameworks, security-by-design, maintainability benchmarks, interoperability, standardized evaluation metrics, and responsible human-AI collaboration.
\end{abstract}
...
An initial seed set of studies was compiled from prior project curation and was subsequently expanded through systematic supplementary searching. This phrasing is used deliberately: the review did not begin from a database-only search, so selection bias from the seed set remains possible and is handled as a validity threat. A supplementary search was conducted on 12 June 2026 across publisher pages, open repositories, and academic indexing surfaces, including arXiv, ACM and ACL Digital Library, SpringerLink, IEEE Xplore, MDPI, Wiley, AIS eLibrary, Google Scholar, and institutional repositories.
...
These base strings were adapted to the syntax requirements of each source where possible. Table~\ref{tab:dbqueries} gives a database-specific rerun protocol, including fields, date range, language, and document-type filters. Because exact database-native exports, hit counts, and timestamps were not retained for every source during the original project work, the table should be read as a reproducible search specification for audit and replication rather than as a complete historical export log. The available evidence package preserves the final 170-record corpus, the final top-50 selection, the extraction workbook, and the generated supplementary CSV files. The supplementary search intentionally emphasized recent work from 2024--2026 because generative AI is rapidly changing the LCNC research landscape, and older literature does not adequately cover LLM-driven development.

\begin{table*}[t]
\centering
\caption{Database-Specific Supplementary Search Protocol for Reproduction}
\label{tab:dbqueries}
\scriptsize
\begin{tabularx}{\textwidth}{p{0.14\textwidth} p{0.50\textwidth} p{0.28\textwidth}}
\toprule
Source & Query expression & Fields and filters \\
\midrule
IEEE Xplore & (``low-code'' OR ``no-code'' OR ``citizen development'') AND (``LLM'' OR ``generative AI'' OR ``automation'' OR ``workflow'' OR ``governance'' OR ``security'' OR ``maintainability'') & All metadata; 2022--2026; English; journals, conferences, early access. \\
ACM Digital Library & (``low-code'' OR ``no-code'' OR ``citizen development'') AND (``large language model'' OR ``LLM'' OR ``generative AI'' OR ``workflow'' OR ``agent'' OR ``security'') & Title, abstract, keywords, full record; 2022--2026; research articles and proceedings. \\
SpringerLink & (``low-code'' OR ``no-code'' OR ``zero-code'') AND (``AI'' OR ``LLM'' OR ``workflow'' OR ``governance'' OR ``maintainability'') & Computer science; 2022--2026; article, conference paper, chapter; English. \\
ScienceDirect / Elsevier & (``low-code'' OR ``no-code'') AND (``software engineering'' OR ``citizen development'' OR ``generative AI'' OR ``governance'') & Title, abstract, keywords; 2022--2026; research and review articles; English. \\
arXiv & all:(``low-code'' OR ``no-code'' OR ``zero-code'') AND all:(``LLM'' OR ``generative AI'' OR ``workflow'' OR ``agent'' OR ``IoT'') & cs.SE, cs.AI, cs.HC where applicable; 2022--2026; preprints. \\
Google Scholar & ``low-code'' OR ``no-code'' with ``LLM'', ``generative AI'', ``citizen development'', ``governance'', ``security'', or ``maintainability'' & 2022--2026; first 100 relevance-ranked results per query family; English-language academic records where available. \\
MDPI / Wiley / AIS eLibrary & (``low-code'' OR ``no-code'' OR ``citizen development'') AND (``AI'' OR ``governance'' OR ``adoption'' OR ``software engineering'') & Site search/title/abstract where supported; 2022--2026; articles, proceedings, and review papers. \\
\bottomrule
\end{tabularx}
\end{table*}
...
Search strings & Base Boolean strings S1--S3 and source-specific rerun queries are reported. \\
Database-specific hit logs & Historical hit counts were not fully preserved; this is disclosed as a limitation. \\
Screening artifacts & 170-record workbook, final top-50 list, PDF manifest, and extraction workbook. \\
Supplementary files & Per-paper appraisal, quote records, theme matrix, claim map, and PRISMA checklist working table. \\
\bottomrule
\end{tabularx}
\end{table}
...
Each included study was scored on 12 criteria: clarity of aim, method fit, sample/data adequacy, context clarity, analytical rigor, validity or reliability checks, evidence-claim alignment, limitations, LCNC relevance, novelty, reproducibility/transparency, and usefulness for the review. These criteria were selected to cover the two evidence types most common in the corpus: empirical socio-technical studies and technical/prototype studies. The criteria therefore combine conventional review-quality concerns, such as aim clarity, method fit, data adequacy, validity, and limitations, with LCNC-specific usefulness, such as relevance, contribution, transparency, and evidence support for synthesis claims. Equal weighting was used because the corpus mixed surveys, case studies, reviews, prototypes, benchmarks, and tool papers; applying method-specific weights would have implied a false precision that the available evidence did not support.

Each criterion was evaluated on a three-point scale (0 = No, 1 = Partially, 2 = Yes), yielding a maximum possible score of 24. Following scoring, studies with scores of 20--24 were categorized as Strong, 15--19 as Moderate, and below 15 as Weak. Stronger claims in this paper rely primarily on studies rated strong, while moderate studies are used cautiously as supporting or illustrative evidence. To address scoring transparency, the full per-paper appraisal table is provided as \texttt{paper\_outputs/supplementary/per\_paper\_quality\_appraisal.csv}. The appraisal is still treated as a structured first-pass assessment rather than a substitute for expert manual review.
...
\textbf{Search and selection limitations.} The review used a PRISMA-informed process but was not a preregistered PRISMA 2020 systematic review. The initial seed set was expanded through supplementary searching, but this design may still introduce selection bias compared with a database-first search. Searches were performed across multiple academic and open sources, but database-specific export logs and exact result counts were not available for every source. The revised paper therefore provides a database-specific rerun protocol and public screening artifacts rather than claiming complete database-level reproducibility.
